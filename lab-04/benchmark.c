...
//void* porque quizas retorna algo *p porque no sabemos de que tipo es la variable
...
void test_thread(int count)
{
    ...
    {
        pthread_t h;
        // id, atributos, funcion, parametro funcion
        pthread_create(&h, NULL, exit_thread, NULL);
        pthread_join(h, NULL);
    }
}
